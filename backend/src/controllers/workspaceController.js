...
export const inviteUserToWorkspace = async (req, res) => {
  try {
    ...
    const invitationLink = `${process.env.FRONTEND_URL}/workspace-invite/${workspace._id}?tk=${invitedToken}`;
...
export const acceptInviteToken = async (req, res) => {
  try {
    ...
    const { user, workspace, role } = decode;
    const workspace = await Workspace.findById(workspaceId);
    ...
    if( inviteInfo.expireAt() < new Date() ){
      return res.status(400).json({
